...
describe("PROVIDER_REGISTRY", () => {
  ...
  it("should have correct default models", () => {
    expect(PROVIDER_REGISTRY.anthropic.defaultModel).toBe(
    ...
    expect(PROVIDER_REGISTRY.ollama.defaultModel).toBe("qwen3.5:4b");
  });
  ...
  it("should have correct env key names", () => {
    ...
    expect(PROVIDER_REGISTRY.google.envKey).toBe(
      "GOOGLE_GENERATIVE_AI_API_KEY"
    );
    expect(PROVIDER_REGISTRY.moonshotai.envKey).toBe("MOONSHOT_API_KEY");
    expect(PROVIDER_REGISTRY.ollama.envKey).toBeUndefined();
  });
});
